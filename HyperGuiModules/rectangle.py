...
class Rectangle:
    def __init__(self, rectangle_frame, listener):
        self.root = rectangle_frame
        ...
        self.checkbox_value = IntVar(value=0)
    ...
    def __update_selected_data_cube(self, event):
        ...
        mask_path = os.path.dirname(dc_path) +'/'+self.listener.output_folder_hypergui +"/MASK_COORDINATES_RECTANGULAR.csv"
    ...
    def _build_info_label(self):
        self.info_label = make_label_button(self.root, text='bp (rectangular)', command=self.__info, width=8)
    ...
    def _build_new_mask_text(self):
        if not self.new_mode:
            string = "Press <k> to set new masks"
        else:
            string = "Press <k> to move last mask"
        inherit_label = make_text(self.root, string, row=27, column=3, columnspan=9, width = 30, bg=tkcolour_from_rgb((BACKGROUND)))
    ...
    def _build_size_inputs(self):
        subFrame = Frame(self.root, width=200, height = 400, bg=tkcolour_from_rgb(BACKGROUND))
        subFrame.grid(row=24, rowspan = 2, column=11, columnspan = 2, padx=10, pady=2)
        
        width_text = make_text(subFrame, content="Width: ", bg=tkcolour_from_rgb(BACKGROUND), column=11, row=24,
                                   width=10)
        self.width_entry = make_entry(subFrame, row=24, column=12, width=5, pady=(5, 5))
        self.width_entry.insert(0, str(self.r_width))
        self.width_entry.bind("<Return>", self.__update_width_and_height)
        height_text = make_text(subFrame, content="Height: ", bg=tkcolour_from_rgb(BACKGROUND), column=11, row=25,
                                   width=10)
        self.height_entry = make_entry(subFrame, row=25, column=12, width=5, pady=(5, 5))
        self.height_entry.insert(0, str(self.r_height))
        self.height_entry.bind("<Return>", self.__update_width_and_height)
        rotation_text = make_text(subFrame, content="Rotation: ", bg=tkcolour_from_rgb(BACKGROUND), column=11, row=26,
                                   width=10)
        self.rotation_entry = make_entry(subFrame, row=26, column=12, width=5, pady=(5, 5))
        self.rotation_entry.insert(0, str(int(round(self.rotation / 0.0174533))))
        self.rotation_entry.bind("<Return>", self.__update_width_and_height)
    ...
    def _build_original_image(self, data):
        if data is None:
            ...
            self.original_image.get_tk_widget().bind('<Key-k>', self.__update_new_mode)
    ...
    def _draw_points(self):
        if self.original_image_data is not None:
            ...
            for point in not_none:
                ...
                self._draw_a_rectangle(not_none[idx], self.r_width, self.r_height, self.rotation, copy_data, color = self.color)
    ...
    def _draw_a_rectangle(self, point, width, height, rotation, image, color = (113,255,66)):
        c0, r0 = point
        start_height = r0-int(height/2)
        start_width = c0-int(width /2)
        start = tuple([start_height, start_width])
        rr, cc = rectangle_perimeter(start, extent = (height, width))
        for i in range(len(rr)):
            r = rr[i]
            c = cc[i]
            c, r = self.rotate((c0, r0), (c,r), rotation)
            image[r % 480, c % 640] = (int(color[0]), int(color[1]), int(color[2]))
        return image
    
    def _remove_noise(self, gray, num):
        Y, X = gray.shape
        nearest_neigbours = [[
            np.argmax(
                np.bincount(
    ...
    def rotate(self, origin, point, angle):
        ...
        return np.round(qx).astype(np.int), np.round(qy).astype(np.int)
    ...
    def __save_coords(self, save_tif = False):
        ...
        output_path2 = path + "/"+self.listener.output_folder_hypergui+"/MASK_COORDINATES_RECTANGULAR" + '.csv'
        output_path3 = path + "/"+self.listener.output_folder_hypergui+"/MASK_COORDINATES" + '.csv'
        if os.path.exists(output_path1) or os.path.exists(output_path2) or os.path.exists(output_path3):
            self.unfocus = True
            yn = messagebox.askquestion ('Verify','Are you sure you want to override mask.csv, MASK_CORDINATES_RECTANGULAR.csv and MASK_COORDINATES.csv?',icon = 'warning')
            if yn == "yes":
                self.unfocus = False
                self.__save_points()
                self.__save_mask()
    ...
    def __add_pt(self, pt):
        if self.coords_list.count((None, None)) != 0:
            if self.new_mode:
                ...
                index = self.coords_list.index((None, None))-1
                if index < 0:
                    index = 0
    ...
    def __save_mask(self):
        points = [point for point in self.coords_list if point != (None, None)]
        img = Image.new('L', (640, 480), 0)
        for point in points:
            data = [[point[1]-int(0.5*self.r_height), point[0]-int(0.5*self.r_width)], [point[1]-int(0.5*self.r_height), point[0]+int(0.5*self.r_width)], [point[1]+int(0.5*self.r_height), point[0]+int(0.5*self.r_width)], [point[1]+int(0.5*self.r_height), point[0]-int(0.5*self.r_width)]]
            data = [self.rotate(point, (_point[1], _point[0]), self.rotation) for _point in data]
        ...
        with open(path + '/'+self.listener.output_folder_hypergui + "/_rectangular" + '.txt', 'w') as fp:
            pass
        
    def __save_points(self):
        data = self.__get_coords_list()
        data = [[float(self.r_height), float(self.r_width)]] + [[float(self.rotation), float(0)]]+ data
        data2 = [[data[2][1]-int(0.5*self.r_height), data[2][0]-int(0.5*self.r_width)], [data[2][1]-int(0.5*self.r_height), data[2][0]+int(0.5*self.r_width)], [data[2][1]+int(0.5*self.r_height), data[2][0]+int(0.5*self.r_width)], [data[2][1]+int(0.5*self.r_height), data[2][0]-int(0.5*self.r_width)]]
        data2 = [self.rotate(data[2], (point[1], point[0]), self.rotation) for point in data2]
        if len(data)>0:
            path = os.path.dirname(self.get_selected_data_cube_path())
            if not os.path.exists(path + '/'+self.listener.output_folder_hypergui):
                os.mkdir(path + '/'+self.listener.output_folder_hypergui)
            output_path = path + '/'+self.listener.output_folder_hypergui + "/MASK_COORDINATES_RECTANGULAR" + '.csv'
            ...
            np.savetxt(output_path2, data2, delimiter=",", fmt="%1.2f")
        else:
            pass
    ...
    def __save_tif(self):
        ...
        mask_img = Image.fromarray(((mask_array*-1+1)*255).astype("uint8"), 'L')
    ...
    def __load_mask(self, path):
        ...
        with open(path) as csvfile:
            ...
            for row in read_csv:
                ...
                ii = ii+1
            csvfile.close()
        for i in range(0, 1000002 - len(coords)):
            coords.append((None, None))
        self.coords_list = coords[2::]
        self.r_height = int(coords[0][0])
        self.r_width = int(coords[0][1])
        self.rotation = coords[1][0]
        self._build_size_inputs()
        self.coords_list_copy = self.coords_list[:]
        self._build_points()
        self._draw_points()
    ...
    def __update_width_and_height(self, event = None):
        self.r_height = int(self.height_entry.get())
        self.r_width = int(self.width_entry.get())
        degree = int(self.rotation_entry.get())%360
        self.rotation = degree * 0.01745
        self._draw_points()
        
    def __update_new_mode(self, event = None):
        self.new_mode = not self.new_mode
        self._build_new_mask_text()
